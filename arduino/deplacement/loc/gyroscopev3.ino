...
const int TURN_SPEED = 80;               // Vitesse de rotation RÉDUITE
const int SLOW_TURN_SPEED = 60;          // Vitesse de rotation fine
...
void navigate() {
  ...
  // 4. Navigation intelligente avec gyroscope
  if (abs(angle_error) > ANGLE_TOLERANCE) {
    // Besoin de tourner
    Serial.print("🔄 Correction ");
    
    // Choisir la vitesse selon l'erreur
    int turn_speed = (abs(angle_error) > 30) ? TURN_SPEED : SLOW_TURN_SPEED;
    ...
    // Attendre un peu que la rotation commence
    delay(100);
...
void testTurning() {
  ...
  turnRight(SLOW_TURN_SPEED);
  
  unsigned long start_time = millis();
  while (millis() - start_time < 10000) {  // Max 10 secondes
    ...
    // Afficher les données du gyroscope aussi
