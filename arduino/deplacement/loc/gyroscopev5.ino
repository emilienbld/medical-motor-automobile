...
const double ANGLE_TOLERANCE = 6.0;      // Tolérance d'angle légèrement augmentée
const int FORWARD_SPEED = 150;           // Vitesse d'avance
const int MIN_TURN_SPEED = 100;          // Vitesse minimum AUGMENTÉE (était 50)
const int MAX_TURN_SPEED = 180;          // Vitesse maximum réduite pour contrôle
...
void setup() {
  ...
  Serial.println("- speed_test : Test des vitesses selon l'angle");
  Serial.println("- turn_test : Test rotation avec feedback gyroscope");
...
void loop() {
  ...
  delay(5);           // Pause très courte pour lecture gyroscope fréquente
}
...
void navigate() {
  ...
  // 4. Navigation intelligente avec vitesse adaptative et temporisation
  if (abs(angle_error) > ANGLE_TOLERANCE) {
    // Besoin de tourner
    
    // Calculer la vitesse selon l'angle à tourner
    int turn_speed = calculateTurnSpeed(angle_error);
    ...
    // Attendre que la rotation se fasse (temps proportionnel à l'angle, réduit)
    unsigned long turn_duration = map(abs(angle_error), 8, 180, 200, 1000);  // Réduit
    delay(turn_duration);
    
    // Arrêter les moteurs après la rotation
    stopMotors();
    delay(150);  // Pause plus longue pour stabiliser
...
void testSpeedMapping() {
  ...
  for (int i = 0; i < num_tests; i++) {
    ...
    int speed = calculateTurnSpeed(angle);
...
void testTurning() {
  ...
  float start_angle = robot_angle;
  float target_angle = normalizeAngle(start_angle + 90);
  ...
  while (millis() - start_time < 15000) {  // Max 15 secondes
    ...
    double angle_error = target_angle - robot_angle;
    angle_error = normalizeAngleDiff(angle_error);
    ...
    float gyro_speed = readGyroZ() - gyro_offset;
    
    Serial.print("Angle: "); Serial.print(robot_angle, 1);
    ...
    // Arrêter si proche de la cible (tolérance plus large)
