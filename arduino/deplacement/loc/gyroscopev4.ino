...
const double ANGLE_TOLERANCE = 5.0;      // Tolérance d'angle (degrés) - plus précis
const int FORWARD_SPEED = 150;           // Vitesse d'avance
const int MIN_TURN_SPEED = 50;           // Vitesse minimum (petits ajustements)
const int MAX_TURN_SPEED = 200;          // Vitesse maximum (grandes rotations)
...
int calculateTurnSpeed(double angle_error) {
  ...
  if (abs_error <= 5) {
    // Très petite correction : vitesse minimale
    speed = MIN_TURN_SPEED;
  } else if (abs_error <= 15) {
    // Petite correction : vitesse progressive
    speed = map(abs_error, 5, 15, MIN_TURN_SPEED, MIN_TURN_SPEED + 30);
  } else if (abs_error <= 45) {
    // Correction moyenne : vitesse modérée
    speed = map(abs_error, 15, 45, MIN_TURN_SPEED + 30, MIN_TURN_SPEED + 80);
  } else if (abs_error <= 90) {
    // Grande correction : vitesse élevée
    speed = map(abs_error, 45, 90, MIN_TURN_SPEED + 80, MAX_TURN_SPEED - 30);
  } else {
    // Très grande correction : vitesse maximale
...
void navigate() {
  ...
  // 4. Navigation intelligente avec vitesse adaptative
  if (abs(angle_error) > ANGLE_TOLERANCE) {
    ...
    // Attendre que la rotation commence
    delay(50);
